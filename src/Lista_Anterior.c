...
//Busca el elemento anterior al elemento ingresado como parámetro y lo retorna
extern ElementoLista *Lista_Anterior(ListaEnlazada *lista, ElementoLista *elemento)
{
	if ((lista != NULL) && (Lista_Vacia(lista) == 0) && (elemento != NULL))
	{
		...
		//Se verifica que el elemento existe y que no sea el primero, ya que no tiene anterior
